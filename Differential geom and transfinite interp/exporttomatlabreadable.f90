...
! \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
	...
	SUBROUTINE exporttomatlabreadable(X, Y,N)
	IMPLICIT NONE
	...
	WRITE(filname, '(a, I0, a)') 'X_' , N , '.txt'
	WRITE(myfmt, '(a, I0, a)') '(' , N+1 , 'f16.10)'

	OPEN(UNIT=out_unit, FILE = filname)
	
	DO i = 0, N 
		write(out_unit, myfmt) (X(i,j), j=0,N)
	END DO
	
	CLOSE(out_unit)

	WRITE(filname, '(a, I0, a)') 'Y_' , N ,  '.txt'

	OPEN(UNIT=out_unit, FILE = filname)
	
	DO i = 0, N 
		write(out_unit, myfmt) (Y(i,j), j=0,N)
	END DO
